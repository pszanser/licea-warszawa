...
def add_metrics(df: pd.DataFrame, P: float, desired_subject: str | None = None) -> pd.DataFrame:
    ...
    if desired_subject:
        ...
        df["ProfileMatch"] = df["PrzedmiotyRozszerzone"].str.contains(pattern, na=False).astype(int)
    else:
...
def compute_composite(df: pd.DataFrame, w: dict | None = None) -> pd.DataFrame:
    ...
    df["Composite"] = (
        w["wQ"]*df["Quality"] + w["wA"]*df["AdmitProb"] +
        w["wC"]*df["CommuteScore"] + w["wP"]*df["ProfileMatch"]
    )
