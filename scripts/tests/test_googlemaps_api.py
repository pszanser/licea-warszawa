...
from api_clients.googlemaps_api import (
    get_travel_time,
...
if not API_KEY:
    print(
        "BŁĄD: Zmienna środowiskowa GOOGLE_MAPS_API_KEY nie jest ustawiona. Testy API nie mogą zostać wykonane."
    )
    exit(1)
...
def test_get_travel_time():
    ...
    result = get_travel_time(
        gmaps, ORIGIN, DESTINATION, mode="transit", departure_time=departure_time
    )
    print(
        f"Czas przejazdu o 7:30 w dzień powszedni: {result} minut"
        if result is not None
        else "Błąd pobierania czasu przejazdu"
    )
