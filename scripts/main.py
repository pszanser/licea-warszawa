...
)  # lub parse_ranking_perspektywy_html
...
def wczytaj_progi():
    ...
    try:
        ...
        df_punkty["SzkolaIdentyfikator"] = df_punkty["NazwaSzkoly"].apply(
            normalize_name
        )
...
def wczytaj_ranking():
    ...
    if not RANKING_EXCEL_FILE.exists() and RANKING_PDF_FILE.exists():
        ...
        logger.error(
            f"BŁĄD: Brak pliku rankingu PDF ({RANKING_PDF_FILE}) lub Excel ({RANKING_EXCEL_FILE})."
        )
...
def polacz_dane(df_vulcan, df_punkty, df_rank):
    ...
    df_merged = pd.merge(
        df_vulcan,
        df_punkty[
            ["NazwaSzkoly", "OddzialNazwa", "Prog_min_klasa", "SzkolaIdentyfikator"]
        ],
        how="left",
...
def oblicz_czasy_dojazdu(df_szkoly):
    ...
    if not api_key:
        logger.warning(
            "OSTRZEŻENIE: Brak klucza GOOGLE_MAPS_API_KEY w zmiennych środowiskowych. Pomijam obliczanie czasów dojazdu i współrzędnych."
        )
        ...
        logger.info(
            f"Pobieranie danych z Google Maps API dla adresu: {ADRES_DOMOWY}..."
        )
        t_api_start = time.perf_counter()
        # łącz nazwy szkół z adresami, aby zwiększyć precyzję geokodowania
        df_szkoly["PelenAdres"] = (
            df_szkoly["NazwaSzkoly"].str.strip()
            + ", "
            + df_szkoly["AdresSzkoly"].str.strip()
        )
        destination_addresses = (
            df_szkoly["PelenAdres"].unique().tolist()
        )  # użyj unikalnych pełnych adresów
        logger.info(
        ...
        for i in range(0, len(destination_addresses), batch_size):
            ...
            logger.info(
                f"  Pobieranie paczki czasów {i // batch_size + 1}/{ (len(destination_addresses) + batch_size - 1) // batch_size } ({len(batch_destinations)} adresów)"
            )
        ...
        if missing_travel_time_count > 0:
            logger.warning(
                f"OSTRZEŻENIE: {missing_travel_time_count} adresów nie ma przypisanego czasu dojazdu."
            )

        # Pobieranie współrzędnych
        logger.info(
            f"  Pobieranie współrzędnych dla {len(destination_addresses)} unikalnych adresów szkół"
        )
        coordinates_dict = get_coordinates_for_addresses_batch(
            gmaps_client, destination_addresses
        )

        df_szkoly["SzkolaLat"] = df_szkoly["PelenAdres"].map(
            lambda addr: coordinates_dict.get(addr, (None, None))[0]
        )
        df_szkoly["SzkolaLon"] = df_szkoly["PelenAdres"].map(
            lambda addr: coordinates_dict.get(addr, (None, None))[1]
        )
        missing_coords_count = (
            df_szkoly["SzkolaLat"].isna().sum()
        )  # Sprawdzamy tylko Lat, Lon powinien być NaN razem z Lat
        if missing_coords_count > 0:
            logger.warning(
                f"OSTRZEŻENIE: {missing_coords_count} adresów nie ma przypisanych współrzędnych."
            )

        t_api_end = time.perf_counter()
        logger.info(
            f"Czas pobierania danych z API (czasy i współrzędne): {t_api_end-t_api_start:.2f} s"
        )

        logger.info(
            f"Zapisywanie czasów dojazdu i współrzędnych do {CZASY_DOJAZDU_FILE}..."
        )
        ...
        logger.info(
            f"Wczytywanie czasów dojazdu i współrzędnych z {CZASY_DOJAZDU_FILE}..."
        )
        try:
            ...
            if "SzkolaIdentyfikator" in df_cache.columns:
                df_szkoly = pd.merge(
                    df_szkoly,
                    df_cache[
                        ["SzkolaIdentyfikator", "CzasDojazdu", "SzkolaLat", "SzkolaLon"]
                    ],
                    how="left",
                    on="SzkolaIdentyfikator",
                )
                ...
                df_szkoly = pd.merge(
                    df_szkoly,
                    df_cache[
                        ["AdresSzkoly", "CzasDojazdu", "SzkolaLat", "SzkolaLon"]
                    ],  # Upewnij się, że te kolumny są w pliku
                    how="left",
                    on="AdresSzkoly",
                )
            ...
            logger.info(
                f"Czas wczytywania danych lokalizacyjnych z pliku: {t_read_end-t_read_start:.2f} s"
            )
        except Exception as e:
            logger.error(
                f"BŁĄD podczas wczytywania lub łączenia danych lokalizacyjnych: {e}"
            )
            df_szkoly["CzasDojazdu"] = None
            df_szkoly["SzkolaLat"] = None
            df_szkoly["SzkolaLon"] = None
        ...
        logger.warning(
            f"Plik {CZASY_DOJAZDU_FILE} nie istnieje i POBIERZ_NOWE_CZASY jest False. Brak danych o czasach dojazdu i współrzędnych."
        )
        df_szkoly["CzasDojazdu"] = None
        df_szkoly["SzkolaLat"] = None
        df_szkoly["SzkolaLon"] = None
...
def zapisz_plik_excel(
    sciezka, df_info, df_szkoly, df_merged, df_rank, df_punkty, df_alg
):
    ...
    try:
        with pd.ExcelWriter(sciezka, engine="openpyxl") as writer:
            ...
            if df_alg is not None and not (
                isinstance(df_alg, pd.DataFrame) and df_alg.empty
            ):
...
def main():
    ...
    szkoly_vulcan = df_vulcan.drop_duplicates(
        subset=[
            "SzkolaIdentyfikator",
            "NazwaSzkoly",
            "AdresSzkoly",
    ...
    df_szkoly = pd.merge(
        szkoly_vulcan, minmax_punkty, how="left", on="SzkolaIdentyfikator"
    )
    t1 = time.perf_counter()
    logger.info(
        f"Czas przygotowania danych do zapisu: {t1-t0:.2f} s"
    )  # 7. Czasy dojazdu
    ...
    df_merged["Profil"] = (
        ...
        .str.extract(r"\[[^\]]+\]\s*([^\(]+)")[
            0
        ]  # wyciągnij tekst po [..] do nawiasu (lub końca)
        .str.strip()
    )
    ...
    df_merged["JezykiObce"] = (
        df_merged["JezykiObce"]
        .str.replace("Pierwszy", "1")
        .str.replace("Drugi", "2")
        .str.replace("Trzeci", "3")
        .str.replace("język", "")
    )
    ...
    if LICZ_SCORE:
        df_alg = add_metrics(
            df_merged, P=CFG["P"], desired_subject=CFG["desired_subject"]
        )
    ...
    for subj in ALL_SUBJECTS:
        ...
        df_merged[subj] = (
            df_merged["PrzedmiotyRozszerzone"]
    ...
    for df in [df_merged, df_szkoly]:
        df["NazwaSzkoly"] = df["NazwaSzkoly"].str.replace(
            "Liceum Ogólnokształcące", "LO", case=False
        )

    # Dodaj kolumnę url
    df_szkoly["url"] = (
        "https://warszawa.edu.com.pl/kandydat/app/offer_school_details.xhtml?schoolId="
        + df_szkoly["IdSzkoly"].astype(str)
    )
    ...
    info_data = {
        "Opis": [
            "Adres początkowy",
            "Data i godzina wyjazdu (szacunkowa)",
            "Uwaga do czasów",
            "szkoly",
            "klasy",
            "ranking",
        ...
        "Wartość": [
            ADRES_DOMOWY,
            departure_str,
            "Szacunki czasów dojazdu mogą się różnić o +/- kilka minut.",
            "lista liceów w Warszawie 2025 z progami oraz rankingiem",
            "lista klas w LO w Wwie 2025 z progami oraz rankingiem",
            "ranking Perspektyw 2025",
            "minimalna progi w roku 2024",
        ],
    }
